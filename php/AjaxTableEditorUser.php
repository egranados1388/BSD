...
class AjaxTableEditorUser
{
	...
	function addRow()
	{
		...
		{
			$html = '';
			$html .= '<form id="'.$this->varPrefix.'_add_edit_form" name="'.$this->varPrefix.'_add_edit_form"  enctype="multipart/form-data"><table '.$this->addTableInfo.'>';
			...
			foreach($this->tableColumns as $col => $info)
			{
				...
				if(stristr($info['perms'],'A') && !isset($info['join']['real_column']) && !isset($info['join']['custom_join']))
				{
					...
					else
					{
						...
						$inputHtml .= '<input type="hidden" id="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'-'.$maximos.'"     '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />'.htmlspecialchars($defVal).'-'.$maximos;  
							...
							if ($col == 'estatus')
							{
							 ...
							 $inputHtml .= '<input hidden="text" id="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />'.htmlspecialchars($defVal).$maxLength.' - Nuevo';
