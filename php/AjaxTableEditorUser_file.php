<?php
/***************************   divison formualarios  en modo usuario************************
...
class AjaxTableEditorUser
{
	...
	function addRow()
	{
		...
		{
			$html = '';
			$html .= '<form id="'.$this->varPrefix.'_add_edit_form"  action="add_ticket.php"  method="post"  name="'.$this->varPrefix.'_add_edit_form"  enctype="multipart/form-data"><table '.$this->addTableInfo.'>';
			...
			foreach($this->tableColumns as $col => $info)
			{
				...
				if(stristr($info['perms'],'A') && !isset($info['join']['real_column']) && !isset($info['join']['custom_join']))
				{
					...
					{
						$inputHtml .= '<input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$inputInfo.' />';						
					}
					...
					{
						$numRows = isset($info['textarea']['rows']) ? $info['textarea']['rows'] : 5;
						$numCols = isset($info['textarea']['cols']) ? $info['textarea']['cols'] : 25;
						$inputHtml .= '<textarea name="'.$this->inputIdPrefix.$col.'" rows="'.$numRows.'" cols="'.$numCols.'" value="'.htmlspecialchars($defVal).'" '.$inputInfo.' >'.htmlspecialchars($defVal).'</textarea>';						
					}
					...
					else if(isset($info['checkbox']))
					{
						...
						$inputHtml .= '<input type="checkbox" name="'.$this->inputIdPrefix.$col.'" value="'.$defVal.'" onclick="updateCheckBoxValue(this,\''.$checkedValue.'\',\''.$unCheckedValue.'\');" '.$checked.'  '.$inputInfo.' />';
					}
					//Join de  tabla
					...
					else if(isset($info['calendar']))
					{
						...
						$inputHtml .= '<span><input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" />'.$fecha_d.'</span>';						
					}
					...
					else
					{
						...
						$inputHtml .= '<input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'-'.$maximos.'"     '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />'.htmlspecialchars($defVal).'-'.$maximos;  
							...
							if ($col == 'estatus')
							{
							  ...
							  $inputHtml .= '<input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' /><img src=images/Estados/cero_nvo.png alt=" Imagen "  height="22" width="22" > (Nuevo)';
									...
									if ($col == 'fecha_entrega')
											...
											$inputHtml .= '<input type="text" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />hgj';
									...
									else  		
											{
														...
														if($col == 'prioridad')
														{
															...
															$inputHtml .= '<input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />'.htmlspecialchars($defVal).''.$maxLength;
														...
														$cone=mysql_connect("172.16.1.1:3306","bideso","");
															mysql_select_db("bd_carto_micro",$cone);
																	$sql="select * from con_tbl_usuario where  status = 1 ";
	...
	$cadena_actual = "<option value='".htmlspecialchars($row[1])." ".htmlspecialchars($row[2])." /".htmlspecialchars($row[0])."'>".htmlspecialchars($row[1])." ".htmlspecialchars($row[2])."</option>";
														...
														$cadena_h=$cadena_h.$cadena_actual;
															...
															$inputHtml .= '<select  name="'.$this->inputIdPrefix.$col.'">';
															
															$users_str=	get_usuariosSU();						
															$inputHtml .= $users_str."</select>";
															...
															else
															{
																	...
																	$inputHtml .= '<input type="hidden" name="MAX_FILE_SIZE" value="3000000"> <input name="'.$this->inputIdPrefix.$col.'" type="file" id="'.$this->inputIdPrefix.$col.'" />';
																	...
																	else
																	{
																				...
																				if($col=='asignado')
																			{
																				...
																				$inputHtml .= '<input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />';
																			...
																			{
																	$inputHtml .= '<input type="text" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />';
																			}
																	}
			...
			$html .= '</table><input type=submit value=Guardar></form><button class="ajaxButton" onclick="toAjaxTableEditor(\'update_html\',\'\');">'.$this->langVars->btnCancel.'</button><div id="addRowButtons">*Cualquier anexo enviado via  correo electronico u otro medio no se contemplara como evidencia  para el optimo seguimiento de  la  solicitud </div>';
				...
				foreach($this->tableColumns as $col => $info)
				{
					if(stristr($info['perms'],'T'))
					{
						...
						else
						{
							...
							$html .= '<td '.$tableCellInfo.'>'.''.str_replace("%"," ",$value).'</td>';
