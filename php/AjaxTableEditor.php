<?php
/********************************** funciones o formularios nivel admin *****************
...
require 'phpmailer/class.phpmailer.php';
 $body= $fecha." - "." Se ha  realizado un nuevo movimiento de ".$tipo." <br><br> ".$descripcion;
...
class AjaxTableEditor
{
	...
	function updateRow()
	{
		...
		if($this->hasRightsToRow($oldPrimaryKeyValue))
		{
			...
			foreach($this->info as $col => $val)
			{
				...
				else
				{
					...
					else
					{
					   ...
					   $valores[] = $this->addTickMarks2($col).":".$val."";
					}
					$afterEditArray[$col] = $val;
				}
			}
			...
			else
			{
				...
				send_mail_(" Modificacion "," solicitante","fecha" ," folio ","".implode(' <br> ',$valores)," de  parte  de ");	
	...
	function addRow()
	{
		...
		{
			$html = '';
			$html .= '<form id="'.$this->varPrefix.'_add_edit_form" name="'.$this->varPrefix.'_add_edit_form"><table '.$this->addTableInfo.'>';
...
mysql_close($con);//cerrar conexion
	...
	// FOrmulario edicion y actualizacion de  registros********************************************************
	function editRow()
	{
		if(stristr($this->permissions,'E'))
		{
			...
			$id = $this->escapeData($this->info);
			$numerox=5;
			$jsUpdateFun = 'updateRow(\''.$id.'\',\''.$this->varPrefix.'\',\''.$numerox.'\');';
			$query = "select * from $this->tableName where $this->primaryKeyCol = '$id'";
			$result = $this->doQuery($query);
			if($row = mysql_fetch_assoc($result))
			{
				...
				foreach($this->tableColumns as $col => $info)
				{
					if(stristr($info['perms'],'E') && !isset($info['join']['real_column']) && !isset($info['join']['custom_join']))
					{
						...
						else
						{
							...
							if  ($this->inputIdPrefix.$col == "asignado")
							{
							
								$inputHtml .= '<select id="'.$this->inputIdPrefix.$col.'"><option value=Andres%Guerrero>Andres  Guerrero<option value=Luis%Enrique%Granados> Luis Enrique Granados  <option value=Mnuel%Razo> Manuel Razo <Option value=Alfonso%Martinez> Alfonso Martinez</select>';
					...
					if(stristr($info['perms'],'T'))
					{
						...
						else
						{
								...
								
								{
									 ...
									 $parlineas = explode("/",$value);
									 $html .= '<td '.$tableCellInfo.'>'.''.$parlineas[0].'</td>';
								}
								...
								ELSE
								{
															...
															
										{
									       ...
									       $cadena_asig = str_replace("%", " ",$value);
										  
										  
										  
										  
										  
										   $cadena_asig2 = startsWith2($cadena_asig ,"L");
										   $cadena_asig3 = startsWith2($cadena_asig ,"A");
										   if ($cadena_asig2==true)
										   {
											 
											 	 $html .= '<td '.$tableCellInfo.'> L E G</td>';
										   ...
										   
										   {
												  ...
												  if ($cadena_asig3==true)
										 		  {
											 
											 	 $html .= '<td '.$tableCellInfo.'> A G </td>';
