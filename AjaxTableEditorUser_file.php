 ...
 */
	...
	function setDefaults()
	{
		...
		$this->start = 0;
		$this->displayNum = isset($_SESSION[$this->varPrefix.'displayNum']) ? $_SESSION[$this->varPrefix.'displayNum'] : 10;
		$this->ascOrDesc = 'Desc';
		$this->orderByColumn = $this->primaryKeyCol;
		//$this->tableTitle = mb_convert_case(str_replace('_',' ',$this->tableName),MB_CASE_TITLE, "UTF-8");
		$this->tableTitle = mb_convert_case("Bitacora  de  Desarrollo  y Modificacion de  SOftware",MB_CASE_TITLE, "UTF-8");
		$this->valError = false;
	...
	function orderColumnsScreen()
	{
		if(stristr($this->permissions,'O'))
		{
			...
			if($this->setMateUserId())
			{
				...
				foreach($this->tableColumns as $col => $info)
				{
					if(stristr($info['perms'],'O') && stristr($info['perms'],'T'))
					{
				...
				if($row = mysql_fetch_assoc($result))
				{
					...
					foreach($this->tableColumns as $col => $info)
					{
						if(stristr($info['perms'],'C') && !isset($info['join']['real_column']))
						{
							...
							{
							   $val = null;
							   $sets[] = $this->addTickMarks($col)." = null";
							}
							else
							{
	...
	function exportInfo()
	{
		...
		$query = $this->selectClause.' '.$this->joinClause.' '.$this->whereClause.' '.$this->groupByInfo.' order by '.$this->addTickMarks($this->orderByColumn).' '.$this->getAscOrDesc().' '.$this->extraOrderByInfo;
		$result = $this->doQuery($query);
	...
	function addRow()
	{
		// Lectura  permiso para  agregar
		if(stristr($this->permissions,'A'))
		{
			$html = '';
			$html .= '<form id="'.$this->varPrefix.'_add_edit_form"  action="add_ticket.php"   name="'.$this->varPrefix.'_add_edit_form"  enctype="multipart/form-data"><table '.$this->addTableInfo.' method = "get">';
			...
			foreach($this->tableColumns as $col => $info)
			{
			     // Permisos  por columna  y join
				if(stristr($info['perms'],'A') && !isset($info['join']['real_column']) && !isset($info['join']['custom_join']))
				{
					$defVal = isset($info['default']) ? $info['default'] : '';
					$inputInfo = isset($info['input_info']) ? $info['input_info'] : '';
					$inputHtml = '';
					
					// Segun la  informacion de   elemento
					
					// Si es  funcion de usuario
					if(isset($info['add_fun']) && is_callable($info['add_fun']))
					{
						$defVal = call_user_func($info['add_fun'],$col,$defVal,$inputInfo);
					}
					
					// Si es funcion de formato
					...
					//textarea
					else if(isset($info['textarea']) || $col == 'Observaciones')
					{
					...
					else if(isset($info['calendar']))
					{
						...
						$inputHtml .= '<span><input type="hidden" id="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" />'.$fecha_d.'</span>';						
					}
					...
					else
					{
						...
						if($col == 'folio')
						{
												...
												//Funcion para Obtener  folio maximo actual
												function get_max()
												{
												...
												$anactual=date('Y');
												$sql="select * from desarrollos where folio like '".$anactual."%' ";
												...
												while ($row=mysql_fetch_array($datos))
																...
																$nombre=$row['folio']; //datos del campo nombre
																$longitud_nombre = strlen($nombre);
																
																// Disercion de  folio segun digitos 
																	if($longitud_nombre == 6)
																		{
																			$subestrin = substr($nombre,-1);
						
																		}
						
																	else
																		{
																			$subestrin = substr($nombre,-2);
						
																		}
					
																	if ($subestrin > $registro )
																		{
																			$registro = $subestrin;
																		}
						...
						if($longitud == 6)
						{
						 ...
						 $maxi = substr($maximo,-2);
						 $maximos = $maxi + 1;
						 
						 }
						
						$inputHtml .= '<input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'-'.$maximos.'"     '.$maxLength.' />'.htmlspecialchars($defVal).'-'.$maximos;  
							...
							if ($col == 'estatus')
							{
							 ...
							 $inputHtml .= '<input type="hidden" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' /><img src=images/Estados/cero_nvo.png alt=" Imagen "  height="22" width="22" >(Nuevo)';
									...
									if ($col == 'fecha_entrega')
									
									        {
											
											$inputHtml .= '<input type="text" id="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />hgj';
											
											//$inputHtml .= '<input type="text" id="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($hoy).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />hgj';
									...
									else  		
											{
														...
														if($col == 'prioridad')
														{
															...
															$inputHtml .= '<input type="hidden" id="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' onKeyPress="if(enterPressed(event)){'.$jsAddFun.' return false;}" '.$inputInfo.' />'.htmlspecialchars($defVal).''.$maxLength;
														...
														else
														{
															...
															if($col == 'solicitante')
															{
																	...
																	function get_usuariosSU()
																		{
	
																			$cone=mysql_connect("carto-micro","root","su235874");
																			mysql_select_db(" bd_carto_micro",$cone);
																			$sql="select * from con_tbl_usuario";
																			...
																			mysql_close($con);//cerrar conexion
																					...
																					//$inputHtml .= '<select  name="you'.$this->inputIdPrefix.$col.'"><option value"Luis%Enrique%Granados%Pastrano">LuisEnriqueGranadosPastrano</option><option value"Luz%Gabriela%Gutierrez">Luz Gabriela Gutierrez</option><option value"Juan%Alberto%Echeveste%Fernandez">Juan Alberto Echeveste  Fernandez</option><option value"Alfonso%Martinez%Irisson">Alfonso Martinez Irisson</option><option value"Luis%Felipe%Villanueva%Gonzalez">Luis Felipe  Villanueva Gonzalez</option><option value"Luis%Gerardo%Gonzalez">Luis Gerardo Gonzalez</option><option value"Juan%Carlos%Canales%Garcia">Juan Carlos Canales  Garcia</option><option value"Veronica Gonzalez">Veronica  Gonzalez</option><option value"Maria Eugenia  Malagon">Maria Eugenia Malagon</option><option value"Alejandro Valdivia">Alejandro Valdivia</option><option value"Jose  de Jesus  Gaytan">Jose  de  Jesus  Gaytan</option><option value"Adriana%Romo">Adriana Romo</option><option value"Ana  Cecilia  Rivera">Ana  Cecilia  Rivera</option><option value"Raul Torres">Raul Torres</option><option value"Juan Rivas">Juan Rivas</option><option value"Susana Edith Centeno">Susana Centeno</option><option value"Leticia Vazquez">Leticia  Vazquez</option><option value"Hector Ivan Cardona">Hector Ivan Cardona</option><option value"Manuel Razo">Manuel Razo</option><option value"Roberto Carlos">Roberto Carlos</option><option value"Marco Pedroza">Marco Pedroza</option></select>';
															}
															else
															{
															        if($col=='archivo')
																	...
																	$inputHtml .= '<input type="hidden" name="MAX_FILE_SIZE" value="100000"> <input name="'.$this->inputIdPrefix.$col.'" type="file" id="'.$this->inputIdPrefix.$col.'" />';
																																																		
																	}
																	else
																	{
																	$inputHtml .= '<input type="text" name="'.$this->inputIdPrefix.$col.'" value="'.htmlspecialchars($defVal).'" '.$maxLength.' />';
																	}
					...
					if(isset($info['calendar']))
					{
								...
								//$calJs[] = $this->getCalJs($info['calendar'],$col,$inputInfo);
					}
					 
				}//fin Avansadas  de  columna permisos
			} // Fin cada  columna
			
			$html .= '</table><input type=submit></form><div id="addRowButtons"><button class="ajaxButton" onclick="'.$jsAddFun.'">'.$this->langVars->btnAdd.'</button><button class="ajaxButton" onclick="toAjaxTableEditor(\'update_html\',\'\');">'.$this->langVars->btnCancel.'</button></div>';
		...
		}// Solo si tiene  permiso de  agregar	
	} // Fin  funcion add row
	...
	function editMultRows()
	{
		if(stristr($this->permissions,'E'))
		{
			if($this->allowEditMult)
			{
				...
				if($row = mysql_fetch_assoc($result))
				{
					$html .= '<form id="'.$this->varPrefix.'_add_edit_form" name="'.$this->varPrefix.'_add_edit_form"><table '.$this->editTableInfo.'>';
					foreach($this->tableColumns as $col => $info)
					{
						if(stristr($info['perms'],'E') && !isset($info['join']['real_column']) && !isset($info['join']['custom_join']) && $col != $this->primaryKeyCol)
						{
					...
					$html .= '</table><input type=submit value="sale"></form><div id="editRowButtons"><button class="ajaxButton" onclick="'.$jsUpdateFun.'">'.$this->langVars->btnUpdate.'</button><button class="ajaxButton" onclick="toAjaxTableEditor(\'update_html\',\'\');">'.$this->langVars->btnCancel.'</button></div>';
				}
	...
	function editRow()
	{
		if(stristr($this->permissions,'E'))
		{
			...
			if($row = mysql_fetch_assoc($result))
			{
				...
				foreach($this->tableColumns as $col => $info)
				{
					if(stristr($info['perms'],'E') && !isset($info['join']['real_column']) && !isset($info['join']['custom_join']))
					{
						if(isset($row[$col]) && strlen($row[$col]) > 0)
						{
	...
	function getCalJs($calInfo,$col,$inputInfo)
	{
		if(is_array($calInfo))
		{
			...
			$inputInfo = strlen(trim($inputInfo)) > 0 ? $inputInfo : '';
		}
		else
		{
		...
		return 'prepareForCalendar(document.getElementById(\''.$this->inputIdPrefix.$col.'\'),\''.$this->inputIdPrefix.$col.'\',\''.$dateFormat.'\','.$resetDate.','.$extraInfo.',\''.addslashes($inputInfo).'\');';
	}
	...
	function getQuickSearchSets($having=false)
	{
		$sets = array();
		foreach($this->tableColumns as $col => $info)
		{
	...
	function getAdvancedSearchSets($having=false)
	{
		$sets = array();
	...
	function getFilterSets($having=false)
	{
		$filterSets = array();
	...
	function getDataFilterSets($having=false)
	{
		// Format data filters
		$dfSets = array();
		foreach($this->tableColumns as $col => $info)
	...
	function formatHavingClause()
	{
		...
		if(!empty($this->havingFilters))
		{
			...
			{
				$this->havingClause .= 'where '.$this->havingFilters.$this->queryLineBreak;
			}
			else
			{
	...
	{
		$this->numResults = 0;
		if(strlen($this->groupByInfo) > 0)
		{
			$query = 'select * from '.$this->tableName.' '.$this->joinClause.' '.$this->whereClause.' '.$this->groupByInfo;
			$result = $this->doQuery($query);
			if($result)
			{
				$this->numResults = mysql_num_rows($result);
			}
		}
		else
		{
	...
	function displayTableHtml()
	{
		...
		$query = $this->selectClause.' '.$this->joinClause.' '.$this->whereClause.' '.$this->groupByInfo.' order by '.$this->addTickMarks($this->orderByColumn).' '.$this->getAscOrDesc().' '.$this->extraOrderByInfo.' limit '.$this->start.', '.$this->displayNum;
		if($this->viewQuery)
		{
			$this->information[] = 'view quey<div id="mateViewQuery" align="left">'.nl2br($query).'</div>';
		}
		$result = $this->doQuery($query);
		if(mysql_num_rows($result) > 0)
		{
			...
			foreach($this->tableColumns as $col => $info)
			{
				...
				{
					$orderCol = isset($info['order_mask']) ? $info['order_mask'] : $col;
					$colHeaderInfo = isset($info['col_header_info']) ? $info['col_header_info'] : '';
					if($this->orderByColumn == $orderCol)
					{
						list($oppAscOrDesc,$arrow) = $this->ascOrDesc == 'asc' ? array('desc','&uarr;') : array('asc','&darr;');
						$html .= '<td '.$colHeaderInfo.' ><a href="javascript: toAjaxTableEditor(\'order_by_changed\', new Array(\''.$orderCol.'\',\''.$oppAscOrDesc.'\'));">'.$info['display_text'].'</a> '.$arrow.'</td>';
					}
					else
					{
						$html .= '<td '.$colHeaderInfo.' ><a href="javascript: toAjaxTableEditor(\'order_by_changed\', new Array(\''.$orderCol.'\',\'asc\'));">'.$info['display_text'].'</a></td>';			
					}
				}
			}
			foreach($this->userColumns as $column)
			{
				$html .= isset($column['title']) ? '-columns<td>'.$column['title'].'</td>' : '<td>&nbsp;</td>';
			}
			...
			while($row = mysql_fetch_assoc($result))
			{
				...
				$html .= '<tr '.implode(' ',$rowInfo).' '.$extraRowInfo.'>'."".$cb;
				if(stristr($this->permissions,'I') && $this->iconColPosition == 'first')
				{
					...
					if(stristr($info['perms'],'T'))
					{
						...
						if($value == 6)
						{
						  ...
						  $alter = 'Concluido';
							...
							$html .= '<td '.$tableCellInfo.'> <img src=images/Estados/'.$imagen.' alt=" Imagen "  height="22" width="22"  ></td>';
		...
		if($this->recordInfo)
		{
			if($this->numResults > 0)
			{
				...
				$start = $this->start + 1;
				$recordHtml = '<div>'.sprintf($this->langVars->ttlDispRecs,'mate',$start,'mate',$end,'mate',number_format($this->numResults)).'</div>';
			}
			else
			{
	...
	function getAdvancedSearchHtml()
	{
		...
		$html .= '<button onclick="handleAdvancedSearch(\''.$this->numAdvSearches.'\');">'.$this->langVars->lblSearch.'</button>&nbsp;<button onclick="toAjaxTableEditor(\'clear_adv_search\',\'\');">'.$this->langVars->btnCSearch.'</button>&nbsp;<button onclick="toAjaxTableEditor(\'add_adv_search\',\'\');">'.$this->langVars->btnAddCrit.'</button>
		</p>
		</div>';
		return $html;
	}
	...
	function AjaxTableEditorUser($tableName,$primaryCol,$errorFun,$permissions,$tableColumns)
	{
		$this->setLangVars();
